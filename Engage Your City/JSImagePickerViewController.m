...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (image != nil) {   // Create a png data image for the PFFile and close the picker
        NSData*   mediaThumbDataFile = UIImageJPEGRepresentation(image, 1.0);
        //  [MBProgressHUD hideHUDForView:self.view.superview animated:YES];
            if ([delegate respondsToSelector:@selector(imagePickerDidSelectImageForType:imageData: andImage:)]) {
                [delegate imagePickerDidSelectImageForType:@"image" imageData:mediaThumbDataFile andImage:image];
            }
   ...
   // [MBProgressHUD showHUDAddedTo:self.view.superview animated:YES];
    ...
    if ([sourceType isEqualToString:@"public.movie"])
    {
        ...
        if (moviePath != nil) {
            ...
            NSData*  mediaThumbDataFile = UIImageJPEGRepresentation(thumbnailImage, 1.0);
                // create the video data file
            NSData*  mediaDataFile = [[NSData alloc] initWithContentsOfURL:movieUrl];
                // Remove the hud
             //   [MBProgressHUD hideHUDForView:self.view.superview animated:YES];
                [picker dismissViewControllerAnimated:YES completion:^{
                    if ([delegate respondsToSelector:@selector(imagePickerDidCreateVideoForType:videoData:videoThumbData:withImage:)]) {
                        [delegate imagePickerDidCreateVideoForType:@"video" videoData:mediaDataFile videoThumbData:mediaThumbDataFile withImage:thumbnailImage];
                    }
                    [self dismissAnimated:YES];
                }];
    ...
    // If the user selected an image
        ...
        if (imageData != nil) {   // Create a png data image for the PFFile and close the picker
         NSData*   mediaThumbDataFile = UIImageJPEGRepresentation(imageData, 1.0);
          //  [MBProgressHUD hideHUDForView:self.view.superview animated:YES];
            [picker dismissViewControllerAnimated:YES completion:^{
                if ([delegate respondsToSelector:@selector(imagePickerDidSelectImageForType:imageData: andImage:)]) {
                    [delegate imagePickerDidSelectImageForType:@"image" imageData:mediaThumbDataFile andImage:imageData];
                }
                [self dismissAnimated:YES];
            }];
