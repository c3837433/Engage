...
//  Origionally Created by Mattieu Gamache-Asselin on 5/15/12.
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        horizontalTextSpace =  [BaseTableViewCell horizontalTextSpaceForInsetWidth:cellInsetWidth];
        ...
        [self.storyButton setBackgroundColor:[UIColor clearColor]];
        ...
        if ([reuseIdentifier isEqualToString:@"ActivityCell"]) {
            ...
            [self.storyButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [self.storyButton setTitleColor:[UIColor colorWithRed:114.0f/255.0f green:114.0f/255.0f blue:114.0f/255.0f alpha:1.0f] forState:UIControlStateHighlighted];
        } else {
            ...
            [self.nameButton setTitleColor:[UIColor colorWithRed:114.0f/255.0f green:114.0f/255.0f blue:114.0f/255.0f alpha:1.0f] forState:UIControlStateHighlighted];
            [self.storyButton setTitleColor:[UIColor colorWithRed:34.0f/255.0f green:34.0f/255.0f blue:34.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
            [self.storyButton setTitleColor:[UIColor colorWithRed:114.0f/255.0f green:114.0f/255.0f blue:114.0f/255.0f alpha:1.0f] forState:UIControlStateHighlighted];
        }
        ...
        [self.storyButton.titleLabel setFont:[UIFont boldSystemFontOfSize:13]];
        [self.storyButton.titleLabel setLineBreakMode:NSLineBreakByTruncatingTail];
        [self.storyButton addTarget:self action:@selector(didTapUserStoryAction:) forControlEvents:UIControlEventTouchUpInside];
        [mainView addSubview:self.storyButton];
...
/* Inform delegate that a user image or name was tapped */
- (void)didTapUserStoryAction:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(cell:didTapStoryButton:)]) {
        [self.delegate cell:self didTapUserButton:self.user];
    }
}
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content {
    return [BaseTableViewCell heightForCellWithName:name contentString:content cellInsetWidth:0];
}
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    ...
    NSString *paddedString = [BaseTableViewCell padString:content withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [BaseTableViewCell horizontalTextSpaceForInsetWidth:cellInset];
...
- (void)setUser:(PFUser *)aUser {
    ...
    if ([Utility userHasProfilePictures:self.user]) {
        [self.avatarImageView setFile:[self.user objectForKey:@"profilePictureSmall"]];
    } else {
...
-(void)setStory:(PFObject *)aStory {
    story = aStory;
    [self.storyButton setTitle:[self.story objectForKey:@"title"] forState:UIControlStateNormal];
    [self.storyButton setTitle:[self.story objectForKey:@"title"] forState:UIControlStateHighlighted];
...
- (void)setContentText:(NSString *)contentString {
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [BaseTableViewCell padString:contentString withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
        if (self.story) {
            CGSize fullNameSize = [self.storyButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                           options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin
                                                                        attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                           context:nil].size;
            NSString * fullPaddedString = [BaseTableViewCell padString:paddedString withFont:[UIFont systemFontOfSize:13] toWidth:fullNameSize.width];
            [self.contentLabel setText:fullPaddedString];
        } else {
...
- (void)setDate:(NSDate *)date {
    ...
    [self.timeLabel setText:timeStampString];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [BaseTableViewCell horizontalTextSpaceForInsetWidth:insetWidth];
    [self setNeedsDisplay];
}
...
- (id<BaseTableViewCellDelegate>)delegate {
    return (id<BaseTableViewCellDelegate>)delegate;
}

- (void)setDelegate:(id<BaseTableViewCellDelegate>)aDelegate {
