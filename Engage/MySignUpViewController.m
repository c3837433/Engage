...
-(IBAction)userStartsRegistering:(UIButton*)button
{
    ...
    if (!haveEmptyFields)
    {
        ...
        newUser[@"UsersFullName"] = userNameField.text;
...
// See which field was clicked on
...
- (void) moveView:(BOOL)outOfWay thisField:(UITextField*)thisField
{
    ...
    // Check which device model is being used
    ...
    switch (field)
    {
            //Set the distance to move the keyboard based on device model, if iPad it needs to move more
        case 0:
            distanceToMove = (isiPad) ? 60 : 0;
            break;
        case 1:
            distanceToMove = (isiPad) ? 110 : 25;
            break;
        case 2:
            distanceToMove = (isiPad) ? 160 : 70;
            break;
        case 3:
            distanceToMove = (isiPad) ? 213 : 135;
            break;
        default:
            break;
    }
