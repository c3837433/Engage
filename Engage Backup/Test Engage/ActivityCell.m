...
//  Origionally Created by Mattieu Gamache-Asselin on 5/14/12.
...
- (void)setActivity:(PFObject *)activity {
    ...
    _activity = activity;
    if ([[activity objectForKey:@"activityType"] isEqualToString:@"follow"] || [[activity objectForKey:@"activityType"] isEqualToString:@"joined"]) {
        [self setActivityImageFile:nil];
    }
    
    NSString *activityString = [ActivityViewController stringForActivityType:(NSString*)[activity objectForKey:@"activityType"]];
    self.user = [activity objectForKey:@"fromUser"];
    ...
    if (self.user && [self.user objectForKey:@"UsersFullName"] && [[self.user objectForKey:@"UsersFullName"] length] > 0) {
        nameString = [self.user objectForKey:@"UsersFullName"];
    }
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [BaseTableViewCell padString:activityString withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
        [self.contentLabel setText:paddedString];
    } else { // Otherwise we ignore the padding and we'll add it after we set the user
        [self.contentLabel setText:activityString];
    }
    Utility* utility = [[Utility alloc] init];
    NSString* timeStampString = [utility stringForTimeIntervalSinceCreated:[activity createdAt]];
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    CGSize nameSize = [name boundingRectWithSize:CGSizeMake(200.0f, CGFLOAT_MAX)
                                      ...
                                      attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                         context:nil].size;
    NSString *paddedString = [BaseTableViewCell padString:content withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
